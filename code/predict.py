...
# x_text_test, _ = data_helper.load_text_and_label('./data/df_testa.csv')
...
def pred_test_set():
    y_test_pred = text_cnn_model.predict_batch(model_file=model_file, x_test=x_dev[:], batch_size=100)
    ...
    df_test_pred.to_csv(path_or_buf='./data/df_testa_pred.csv', header=True, index=False)    # write out testa dataset
