    ...
    def build_model(self):
        ...
        with tf.device(self.device_name):
            ...
            with tf.name_scope('embedding'):
                ...
                self.embedded_words_expanded = tf.transpose(self.embedded_words_expanded, [0, 1, 3, 2])
            ...
            for i, filter_size in enumerate(self.filter_sizes):
                ...
                with tf.name_scope('conv-%s' % filter_size):
                    ...
                    filter_shape = [filter_size, 1, self.embedding_size, self.num_filters]
                    W = tf.Variable(tf.truncated_normal(shape=filter_shape, mean=0.0, stddev=0.1), name='W_conv1')
                    b = tf.Variable(tf.constant(value=0.1, shape=[self.num_filters]), name='b_conv1')
                    conv = tf.nn.conv2d(
                    ...
                    h = tf.layers.batch_normalization(h, axis=-1)

                    filter_shape = [filter_size, 1, self.num_filters, self.num_filters]
                    W = tf.Variable(tf.truncated_normal(shape=filter_shape, mean=0.0, stddev=0.1), name='W_conv2')
                    b = tf.Variable(tf.constant(value=0.1, shape=[self.num_filters]), name='b_conv2')
                    conv = tf.nn.conv2d(
                        input=h,
                        filter=W,
                        strides=[1, 1, 1, 1],
                    ...
                    filter_shape = [filter_size, 1, self.num_filters, self.num_filters]
                    W = tf.Variable(tf.truncated_normal(shape=filter_shape, mean=0.0, stddev=0.1), name='W_conv3')
                    b = tf.Variable(tf.constant(value=0.1, shape=[self.num_filters]), name='b_conv3')
                    conv = tf.nn.conv2d(
                        input=h,
                        filter=W,
                        strides=[1, 1, 1, 1],
